  ...
  if (ticket) {
    return (
      <div className="relative h-full w-full overflow-hidden bg-[#e9eef1] text-[#172027]">
        {children}
        <motion.div
          className="absolute bottom-[clamp(10px,2vh,24px)] left-1/2 z-30 w-max max-w-[92%] -translate-x-1/2 rounded-full border border-[#cbd5da] bg-white/90 px-5 py-2 text-center text-base font-light tracking-[0.2em] text-[#4d5e68] shadow-sm"
    ...
    <div className="relative h-full w-full overflow-hidden text-white">
      {phase === "outfit" ? (
        <>
          ...
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_0%,transparent_66%,rgba(0,0,0,0.28)_100%)]" />
        </>
      ) : (
        <>
      ...
      <div className="absolute top-[clamp(12px,2.5vh,24px)] right-[clamp(14px,3vw,32px)] left-[clamp(14px,3vw,32px)] z-30 flex items-center justify-between gap-3">
        <span className="text-xs font-light tracking-[0.32em] whitespace-nowrap text-white/70 max-[480px]:tracking-[0.18em]">
          DAILY REHEARSAL
        </span>
        <div className="flex items-center gap-3">
          <span className="text-xs font-light tracking-[0.16em] whitespace-nowrap text-white/60 max-[480px]:hidden">
            ...
            {PHASES.map((item, index) => (
              <div
                ...
                className={`h-1.5 w-[clamp(14px,2.6vw,28px)] rounded-full ${
                  index <= phaseIndex ? "bg-white/70" : "bg-white/15"
                }`}
              />
            ))}
          </div>
