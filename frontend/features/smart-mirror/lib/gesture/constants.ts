...
// 좌표는 MediaPipe 정규화 좌표(0~1, 카메라 프레임 폭 대비), 시간은 ms.
// 주의: 기준이 브라우저 창이 아니라 카메라가 찍는 프레임이다 — 창 크기·줌은
// 무관하고, 사용자가 카메라에서 멀수록 같은 값이라도 더 큰 동작을 요구한다.
 ...
 * 0.2 → 0.25(윈도우 연장 보정) → 0.13: 손목 축 회전 "까닥" 스와이프는
 * 진폭이 작아 추적점을 손끝(landmark 12)으로 바꾸면서 내렸다
 * (2026-08-11 디버그 오버레이로 실측 튜닝). 스와이프 오발사는 팜홀드
 * 확정이 지켜주지만 브리핑·시뮬레이션의 PREV(발화 후보 취소)는 예외임에
 * 유의 — 가만있는데 넘어가는 오탐이 보이면 0.15부터 되올릴 것.
 */
export const SWIPE_MIN_DISTANCE = 0.13;
/**
...
/**
 * 인식 입력을 "화면에 보이는 영역"으로 자를 때 바깥으로 더 두는 여유 비율.
 * 전시장 카메라는 세로 모니터보다 훨씬 넓게 찍어서, 자르지 않으면 화면 밖에
 * 서 있는 관람객의 손이 인식 후보로 들어온다 (2026-09-14 현장).
 * 0.08은 화면 가장자리에 걸친 손이 잘려 인식이 끊기는 것만 막는 최소치.
 * 옆사람 손이 계속 잡히면 음수(-0.2 등)로 내려 보이는 영역보다 더 좁히고,
 * 내 손이 자꾸 끊기면 올린다. 조정 결과는 디버그 오버레이(D)의 점선 경계로
 * 바로 보인다.
 */
export const GESTURE_CROP_MARGIN = 0.08;

/**
 * 잘라낸 영역을 인식에 넘기기 전에 줄이는 긴 변 상한(px).
 * 모델 내부 입력은 어차피 훨씬 작아서 원본 크기를 그대로 넘길 이유가 없고,
 * 매 프레임 drawImage 비용만 커진다.
 */
export const GESTURE_INPUT_MAX_SIDE = 720;
 ...
 * 공백이 더 길다 (2026-08-11). SWIPE_WINDOW_MS보다 짧게 유지할
 ...
 * 문턱을 낮춰 긴가민가한 손을 계속 추적한다 (2026-08-11).
