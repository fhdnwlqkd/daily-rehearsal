...
DB는 context slot과 option 같은 구조화 설정을 저장하고, 이 prompt 파일들은 LLM의 역할, 출력 형식, 말투, 금지 사항을 정의한다.

MVP prompt 파일:

- `context-extraction.md`: 사용자 브리핑에서 context slot 값을 추출한다.
- `follow-up-question.md`: 부족한 필수 slot을 1회 묶음 질문으로 만든다.
- `simulation-dialogue.md`: `situation_type`, `critical_moment`, 사용자 맥락에 맞는 리허설 장면을 만든다.
